    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <div className="mb-12">
        <div className="flex items-center gap-0.5 mb-4">
          <span className="text-3xl font-black font-mono text-white">HACK</span>
          <span className="text-3xl font-black font-mono text-[#00FF88]">WIRE</span>
        </div>
        <h1 className="text-4xl font-bold font-mono text-white mb-4">
          Cybersecurity News, <span className="text-[#00FF88]">Decoded</span>
        </h1>
        <div className="h-px bg-gradient-to-r from-[#00FF88]/30 to-transparent mb-6" />
      ...
      {/* Mission */}
      <section className="bg-[#0F0F1A] border border-[#1E1E2E] rounded-xl p-8 mb-10">
        <div className="flex items-center gap-2 mb-4">
          <span className="text-[#00FF88] font-mono text-sm">▶</span>
          <h2 className="text-[#00FF88] font-mono font-bold text-sm tracking-widest uppercase">Our Mission</h2>
        </div>
      ...
      <section className="mb-10">
        <h2 className="text-xl font-bold font-mono text-white mb-6">What We Cover</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {categories.map((cat) => (
            <div key={cat.label} className="bg-[#0F0F1A] border border-[#1E1E2E] rounded-lg p-4">
              <div className="flex items-center gap-2 mb-2">
                <span>{cat.icon}</span>
                <span className="text-white font-mono font-bold text-sm">{cat.label}</span>
              </div>
      ...
      {/* Sources */}
      <section className="mb-10">
        <h2 className="text-xl font-bold font-mono text-white mb-6">Sources We Aggregate From</h2>
          ...
          <div className="divide-y divide-[#1E1E2E]">
            {sources.map((source) => (
              <div key={source.name} className="flex items-center justify-between px-5 py-3">
                <span className="text-gray-300 font-mono text-sm">{source.name}</span>
                <span className="text-gray-600 text-xs font-mono bg-[#1A1A2A] px-2 py-0.5 rounded">
                  {source.type}
                </span>
              </div>
            ))}
      ...
      <div className="text-center">
        <Link
          href="/"
          className="inline-flex items-center gap-2 px-6 py-3 border border-[#00FF88]/40 text-[#00FF88] font-mono font-semibold rounded-lg hover:bg-[#00FF88]/10 transition-all duration-200"
        >
          ← Back to Latest News
        </Link>
      </div>
